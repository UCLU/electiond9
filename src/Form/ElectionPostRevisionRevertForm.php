...
/**
 * Provides a form for reverting a Election post revision.
 *
 * @ingroup election
 */
class ElectionPostRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'election_post_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Election post: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Election post %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\election\Entity\ElectionPostInterface $revision
   ...
   */
  protected function prepareRevertedRevision(ElectionPostInterface $revision, FormStateInterface $form_state) {
