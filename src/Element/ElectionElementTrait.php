...
namespace Drupal\election\Element;

use Drupal\Core\Form\FormStateInterface;
use Drupal\Core\Render\Element;

/**
 * Allows form elements to use #election_element_submit.
 *
 ...
 */
trait ElectionElementTrait {

  /**
   * Attaches the #election_element_submit functionality.
   *
   * @param array $element
  ...
  public static function attachElementSubmit(array $element, FormStateInterface $form_state, array &$complete_form) {
    if (isset($complete_form['#election_element_submit_attached'])) {
      return $element;
    }
    ...
    $complete_form['#election_element_submit_attached'] = TRUE;

    return $element;
  }

  /**
   * Confirms that #election_element_submit handlers can be run.
   *
   * @param array $element
   *   The form element.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  /**
   * Submits elements by calling their #election_element_submit callbacks.
   *
   * Form API has no #element_submit, requiring us to simulate it by running
   * the #election_element_submit handlers either in the last step of
  ...
  /**
   * Checks whether #election_element_submit callbacks should be executed.
   *
   * @param \Drupal\Core\Form\FormStateInterface $form_state
   *   The form state.
   *
  ...
  /**
   * Calls the #election_element_submit callbacks recursively.
   *
   * @param array &$element
   *   The current element.
   * @param \Drupal\Core\Form\FormStateInterface $form_state
  ...
  protected static function doExecuteSubmitHandlers(array &$element, FormStateInterface $form_state) {
    ...
    if (!empty($element['#election_element_submit'])) {
      foreach ($element['#election_element_submit'] as $callback) {
